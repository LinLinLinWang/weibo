...
* @version 创建时间：2018年12月27日 下午3:41:41
* 类说明
*/
/**
* @author 王林
...
@ServerEndpoint(value="/websocket/{userno}",configurator = SpringConfigurator.class)
...
public void sendToUser(String message) {
 ...
 if (webSocketSet.get(sendUserno) != null) {
	 System.out.println("发送者"+userno);

	 
	 
	 if(null==userServiceImpl.user_exist_returnUser(userno)){
		 whogive="无名氏";
		 
	 }else{
		 whogive=userServiceImpl.user_exist_returnUser(userno).getuName();
  ...
  webSocketSet.get(sendUserno).sendMessage(now + "用户手机号"+userno+"用户名字"+whogive+"发来消息"+sendMessage);
  
  ChatWithPeople chatWithPeople=new ChatWithPeople();
  chatWithPeople.setContent(sendMessage);
  chatWithPeople.setReceivePhone(sendUserno);
  chatWithPeople.setSendPhone(userno);
  ...
  //保存聊天内容
      if(1==chatServiceImp.insertChatDate(chatWithPeople)){
    	  ...
    	  System.out.println("个人中心收到的消息保存成功");
      }
